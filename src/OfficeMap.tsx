...
import { DesIcon, DevIcon, HomeIcon, OfficeIcon, PmIcon } from "./LeafletIcons";
      ...
      <LayersControl position="bottomleft">
        <LayersControl.Overlay checked={isFilterChecked("RemoteIcon")} name="Remote">
          <LayerGroup eventHandlers={checkboxEventHandlers("RemoteIcon")}>
            {offices
              .filter((o) => o.isRemoteOrHomeOffice())
              .filter((o) => o.employees.length !== 0)
              .map((office) => getMarkerComponent(office, HomeIcon))}
          </LayerGroup>
        </LayersControl.Overlay>

        <LayersControl.Overlay checked={isFilterChecked("OfficeHubsIcon")} name="Office Hubs">
          <LayerGroup eventHandlers={checkboxEventHandlers("OfficeHubsIcon")}>
            {offices
              .filter((o) => !o.isRemoteOrHomeOffice())
              .filter((o) => o.employees.length !== 0)
              .map((office) => getMarkerComponent(office, OfficeIcon))}
          </LayerGroup>
        </LayersControl.Overlay>
